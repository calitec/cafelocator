...
const InfoDetailPresenter: React.FunctionComponent<IInfoDetailPresenterProps> = ({
    ...
    return (
        <>
            {transY < 1
                ? <div css={infoDetailContainer} className={mapDetail ? 'infoDetail activated' : 'infoDetail'}>
                    <button onClick={onClearDirections}><FontAwesomeIcons icon={'close'} color={'black'} /></button>
                    <div className='infoDetail__wrapper' ref={refs}>
                        <Slick photos={mapDetail.photos} />
                        <div className='infoDetail__description'>
                            <h3>
                                {mapDetail && mapDetail.name}
                            </h3>
                            <button className="travel" onClick={setTravel}>경로</button>
                            <div className='rating head'>
                                <span className='icon'>
                                    <Rating star={mapDetail.rating} single={true} />
                                </span>
                                <span className='indent'> {mapDetail && mapDetail.rating || '별점 없음'}</span>
                            </div>
                            <div className='head'>
                                <span className='icon'><FontAwesomeIcons icon={'phone'} color={'black'} /> </span>
                                <span className='indent'>{mapDetail && mapDetail.formatted_phone_number || '전화번호 없음'}</span>
                            </div>
